...
%          fm.c_args.log_args is true); 1-by-ncols string array
    ...
    inputs.M.msg = input_msg(inputs.M, "M", "N", pad_n);
    inputs.O.msg = input_msg(inputs.O, "O", "N_pump", pad_n);
    inputs.chi.msg = input_msg(inputs.chi, "chi", "1", pad_n);
    inputs.f0.msg = compose("f0: (N_f-by-1) [%s]\n", f_unit);
    inputs.X_probe.msg = compose("X_probe: (N_probe-by-2) [%s]\n", length_unit);
end
